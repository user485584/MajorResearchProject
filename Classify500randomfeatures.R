...
for (start_index in seq(35501, total_features, by = chunk_size)) {
  ...
  results <- train_classifier(TrainData, TestData, Testmetadata, "all", PerCelltype = TRUE, foldNr = i, Analysis = Analysis, preloaded_models = NULL, ROCoutput_dir = "/hpc/hers_en/rballieux/AlzheimersFinal/500randomfeatures/ROCplots/")
  ...
  save_objects(results, fold_number = start_index, path_prefix = "/hpc/hers_en/rballieux/AlzheimersFinal/500randomfeatures/")
...
write.csv(all_results, "/hpc/hers_en/rballieux/AlzheimersFinal/500randomfeatures/ClassifierResults500randomfeats.csv", row.names = FALSE)