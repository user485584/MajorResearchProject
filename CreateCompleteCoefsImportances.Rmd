...
# Retrieve mapping from Ensembl to Entrez IDs
...
# Retrieve mappings between Ensembl IDs, Gene Symbols, and Entrez IDs
...
# Rename the columns for easier joining
additional_mapping <- additional_mapping %>%
...
```{r}
reduced_importances <- all_importances %>%
  group_by(Disease, Analysis, Fold, Classifier, Run) %>%
  arrange(desc(absimportance)) %>%  # Arrange in descending order of absimportance
  slice_head(n = 50) %>%  # Take the top 50 genes per group
...
folder_path <- "/Users/rutger/Desktop/Bureaublad/School/Master/Stage/Results/ResultFiles/Importances/"
write.csv(all_importances, file = paste0(folder_path, "All_importances.csv"), row.names = FALSE)
write.csv(reduced_importances, file = paste0(folder_path, "Reduced_importances.csv"), row.names = FALSE)
```
