...
config <- list(
  ...
  chunk_size = 2000,                                                                     # Number of features to process per chunk
  OutputFolder = "/hpc/hers_en/rballieux/ALS_FTLD/C9ALS/2000randomfeatures/"           # Base folder for all outputs
)
...
for (start_index in seq(start, total_features, by = config$chunk_size)) {
  ...
  results$Analysis <- "2000RandomFeatures"
...
write.csv(all_results, paste0(config$OutputFolder, Disease, "ClassifierResults2000randomfeats.csv"), row.names = FALSE)